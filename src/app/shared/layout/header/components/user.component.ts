...
import { ProfileServiceProxy, TenantLoginInfoDto,DonViCoSoServiceProxy } from '@shared/service-proxies/service-proxies';
import { EditDonViCoSoComponent } from '../../../../pages/don-vi-co-so/danh-sach/edit-don-vi-co-so.component';
...
@Component({
    ...
    <nz-dropdown-menu #userMenuTpl="nzDropdownMenu">
        <ul nz-menu>
            <li *ngIf="appSession.user.donViCoSoId >0" nz-menu-item (click)="changeInfo(appSession.user.donViCoSoId)">
...
export class HeaderUserComponent extends AppComponentBase implements OnInit {
    ...
    changeInfo(coSoId): void {
        this._donViCoSoService.getById(coSoId).subscribe(result => {
            this.modalHelper.create(EditDonViCoSoComponent, { dataItem: result }).subscribe(result => {
