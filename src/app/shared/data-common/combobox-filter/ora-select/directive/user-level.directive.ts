...
@Directive({
    selector: '[dirUserLevel]',
    providers: [
        {
            provide: SelectOptions,
            useExisting: UserLevelDirective,
        },
    ],
})
export class UserLevelDirective extends AppComponentBase implements ISelectOptions {
    ...
    getDataSourceFromServer(): Observable<ISelectOption[]> {
        if (this.appSession.user.level === this.userLevel.SA || this.appSession.user.level === null || this.appSession.user.level === 0) {
        ...
        } else if (this.appSession.user.level === this.userLevel.BO_Y_TE) {
            return of<ISelectOption[]>([
                { value: this.userLevel.BO_Y_TE, displayText: 'Bộ y tế' },
                { value: this.userLevel.QUAN_LY_VUNG, displayText: 'Quản lý vùng' },
                { value: this.userLevel.SO_Y_TE, displayText: 'Sở y tế' },
