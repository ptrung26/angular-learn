...
export class QuocTichComboComponent extends AppComponentBase implements OnInit, OnDestroy, ControlValueAccessor {
    ...
    constructor(
        injector: Injector,
        private _comboboxService: DanhMucQuocTichServiceProxy,
        ...
        this.$maTinh.pipe(
            switchMap((ma) => {
                if (ma) {
                    return this._dataCommonService.getComboboxDataObs('quoctich', this._comboboxService.comboBoxData());
                } else {
                    return of([]);
                }
            }),
