...
import {
    ...
    DanhMucTinhServiceProxy, USER_LEVEL,
...
const VALUE_ACCESSOR: Provider = {
    ...
    useExisting: forwardRef(() => BenhVienComboComponent),
    multi: true,
};

@Component({
    selector: 'benh-vien-combo',
    template: `
...
export class BenhVienComboComponent extends AppComponentBase implements OnInit, OnDestroy, ControlValueAccessor {

    @Input() isDeleteTextDonViCoSo = false;
    ...
    constructor(
        injector: Injector,
        private _comboboxService: DonViCoSoServiceProxy,
        ...
        this.$maTinh.pipe(
            switchMap((ma) => {
                if (ma) {
                    return this._dataCommonService.getComboboxDataObs('benhvien_' + ma, this._comboboxService.comboBoxDataBenhVien(0, ma));
                } else {
                    return of([]);
                }
            }),
            ...
            const itemFind = this.optionListSource.find(item => item.value === this.value.toString());
            if (!itemFind) {
                this.value = null;
    ...
    onChangeValue(event: any): void {
        this.onChange(event);
        let obj = this.optionList.find(x => x.value === event);
        this.onItemSelected.emit(obj?.displayText);
    }
