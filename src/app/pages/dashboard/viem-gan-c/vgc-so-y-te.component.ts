...
@Component({
    selector: 'vgc-so-y-te',
    templateUrl: './vgc-so-y-te.component.html',
    encapsulation: ViewEncapsulation.Emulated,
})
export class VgcSoYTeComponent extends AppComponentBase implements OnInit, OnChanges {
    @Input() maTinh: string;
    ...
    vgcsytscreeningtestlabel = [];
    vgcsytscreeningtestpositivelable = [];
    tyleucchevirus = 0;
    ...
    loadData(): void {
        ...
        this.vgcsytscreeningtestlabel = ['0', '0', '0', '0'];
        this.vgcsytscreeningtestpositivelable = ['0', '0', '0', '0'];
        ...
        this._dataService.dashboardTongHop(tongHopRequest)
            ...
            .subscribe(result => {
                ...
                this.sangLocDuongTinh = this.dataTongHop.filter(x => x.antiHCV_KetQua === KET_QUA_XET_NGHIEM.DUONG_TINH).length;
                if (this.dataTongHop.length > 0) {
                    this.sangLocDuongTinhPercent = this.sangLocDuongTinh * 100 / this.dataTongHop.length;
                }
                this.xetNghiem_ChanDoan = this.dataTongHop.filter(x => (x.hcvarN_NgayLam !== null && x.hcvarN_NgayLam !== undefined)).length;
                this.nhiemMan_ChanDoan = this.dataTongHop.filter(x => x.viemGanCMan === true && x.antiHCV_KetQua === KET_QUA_XET_NGHIEM.DUONG_TINH).length;
                    ...
                    this.groupByCoSoSort.push(
                        {
                            ...
                            soDuongTinh: this.groupByCoSo[x].filter(x => x.antiHCV_KetQua === KET_QUA_XET_NGHIEM.DUONG_TINH).length,
                            soAmTinh: this.groupByCoSo[x].filter(x => x.antiHCV_KetQua === KET_QUA_XET_NGHIEM.AM_TINH).length,
                            soNhiemMan: this.groupByCoSo[x].filter(x => x.antiHCV_KetQua === KET_QUA_XET_NGHIEM.DUONG_TINH && (x.hcVcAg_KetQua === KET_QUA_XET_NGHIEM.DUONG_TINH || x.hcvarN_Nguong === NHOM_NGUONG.TREN_NGUONG)).length,
                            soDieuTri: this.groupByCoSo[x].filter(x => x.viemGanCMan === true && x.soLanDieuTri_ViemGanC > 0 && x.antiHCV_KetQua === KET_QUA_XET_NGHIEM.DUONG_TINH && (x.hcVcAg_KetQua === KET_QUA_XET_NGHIEM.DUONG_TINH || x.hcvarN_Nguong === NHOM_NGUONG.TREN_NGUONG)).length,
                        },
                    );
                });
                this.groupByCoSoSort = _.sortBy(this.groupByCoSoSort, 'soBenhNhan');

                let vgcsytscreeningtestNam = _.cloneDeep(this.dataTongHop).filter(x => x.gioiTinh === GIOI_TINH.NAM).length;
                let vgcsytscreeningtestNu = _.cloneDeep(this.dataTongHop).filter(x => x.gioiTinh === GIOI_TINH.NU).length;
                if (this.dataTongHop.length > 0) {
                    this.vgcsytscreeningtestlabel = [];
                    let vgcsytscreeningtestNamPercent = (vgcsytscreeningtestNam * 100 / this.dataTongHop.length).toFixed(1);
                    this.vgcsytscreeningtestlabel.push(vgcsytscreeningtestNamPercent);
                    this.vgcsytscreeningtestlabel.push(vgcsytscreeningtestNu);
                    this.vgcsytscreeningtestlabel.push((100 - parseFloat(vgcsytscreeningtestNamPercent)).toFixed(1));
                }
                ...
                let vgcsytscreeningtestpositiveNam = dataScreeningPositive.filter(x => x.gioiTinh === GIOI_TINH.NAM).length;
                let vgcsytscreeningtestpositiveNu = dataScreeningPositive.filter(x => x.gioiTinh === GIOI_TINH.NU).length;
                if (this.dataTongHop.length > 0) {
                    this.vgcsytscreeningtestpositivelable = [];
                    this.vgcsytscreeningtestpositivelable.push(vgcsytscreeningtestpositiveNam);
                    let vgcsytscreeningtestpositiveNamPercent = (vgcsytscreeningtestpositiveNam * 100 / dataScreeningPositive.length).toFixed(1);
                    this.vgcsytscreeningtestpositivelable.push(vgcsytscreeningtestpositiveNamPercent);
                    this.vgcsytscreeningtestpositivelable.push(vgcsytscreeningtestpositiveNu);
                    this.vgcsytscreeningtestpositivelable.push((100 - parseFloat(vgcsytscreeningtestpositiveNamPercent)).toFixed(1));
                }
                let topCoSoData = [];
                let countTop = 0;
                ...
                for (let i = this.groupByCoSoSort.length - 1; i > -1; i--) {
                    if (this.groupByCoSoSort[i]['soNhiemMan'] > 0 || this.groupByCoSoSort[i]['soDieuTri'] > 0) {
                        topCoSoNhiemManDieuTri.push(
                            {
                                'city': this.groupByCoSoSort[i]['tenCoSo'] + '\n(' + this.groupByCoSoSort[i]['soNhiemMan'] + ')',
                                'person': this.groupByCoSoSort[i]['soNhiemMan'],
                                'treatment': this.groupByCoSoSort[i]['soDieuTri'],
                                'notreatment': this.groupByCoSoSort[i]['soNhiemMan'] - this.groupByCoSoSort[i]['soDieuTri'],
                            },
                        );
                    }
                }
                this.getChart(this.dataTongHop, this.dataTongHopBHYT, this.groupByCoSoSort, topCoSoData, topCoSoNhiemManDieuTri, this.tongSangLoc, this.sangLocDuongTinh, this.VGC_XetNghiemKhangDinh, this.VGC_XetNghiemKhangDinhChuanDoan);
                abp.ui.clearBusy();
            });
    }
    ...
    getChart(dataTongHop, dataTongHopBHYT, groupByCoSoSort, topCoSoData, topCoSoNhiemManDieuTri, tongSangLoc, sangLocDuongTinh, VGC_XetNghiemKhangDinh, VGC_XetNghiemKhangDinhChuanDoan) {
        am4core.ready(function() {
            am4core.useTheme(am4themes_animated);
            //#region Số người xét nghiệm anti-HCV

            let chartScreeningTest = am4core.create('vgc-syt-screening-test', am4charts.XYChart);
            ...
            let chartScreeningPositive = am4core.create('vgc-syt-screening-test-positive', am4charts.XYChart);
            ...
            malePositiveLabel.label.align = 'center';
            ...
            //#region 10 cơ sở có xét nghiệm anti-HCV cao nhất

            let chartTopHospital = am4core.create('vgc-syt-chart-top-hospital', am4charts.XYChart);
            ...
            let chartSexTest = am4core.create('vgc-syt-chart-sex-cag-test', am4charts.PieChart);
            ...
            let chartOldTest = am4core.create('vgc-syt-chart-old-cag-test', am4charts.XYChart);
            chartOldTest.data = [{
                'old': '<15',
            ...
            let chartSexPositive = am4core.create('vgc-syt-chart-sex-cag-positive', am4charts.PieChart);
            ...
            let chartOldPositive = am4core.create('vgc-syt-chart-old-cag-positive', am4charts.XYChart);
            ...
            let chartBeginTreatment = am4core.create('vgc-syt-chart-beingtreatment', am4charts.PieChart);
            ...
            //#region Số người bệnh điều trị viêm gan C theo cơ sở

            let chartInfectionTreatment = am4core.create('vgc-syt-chart-infection-treatment', am4charts.XYChart);
            ...
            // let chartSexCure = am4core.create('vgc-syt-chart-sex-cure', am4charts.PieChart);
            ...
            //
            // let chartOldCure = am4core.create('vgc-syt-chart-old-cure', am4charts.XYChart);
            // chartOldCure.data = [{
            //     'old': '<15',
            ...
            let chartCure = am4core.create('vgc-syt-chart-cure', am4charts.PieChart);
            // let sexCureDate = dataTongHop.filter(x => x.viemGanCMan === true && (x.ngayKetThuc_ViemGanC !== null && x.ngayKetThuc_ViemGanC !== undefined) && x.hcvarN_LanCuoi_Nguong === NHOM_NGUONG.DUOI_NGUONG && x.soNgayDieuTriViemGanC > 83);
            ...
            let chartMultiLevel = am4core.create('vgc-syt-chart-multi-level', am4charts.XYChart);
            ...
            let chartTreatmentInsurance = am4core.create('vgc-syt-chart-treatment-insurance', am4charts.PieChart);
